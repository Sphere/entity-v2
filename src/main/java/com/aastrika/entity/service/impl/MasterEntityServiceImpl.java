...
public class MasterEntityServiceImpl implements MasterEntityService {
    ...
    public List<MasterEntity> parseEntitySheet(MultipartFile entitySheet) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(entitySheet.getInputStream()));
            ...
            for (CSVRecord record : csvParser) {
                ...
                String generatedCode = record.isMapped(EntitySheetHeaders.CODE) && !record.get(EntitySheetHeaders.CODE).isBlank()
                ...
                if (record.isMapped(EntitySheetHeaders.CREATED_DATE) && !record.get(EntitySheetHeaders.CREATED_DATE).isBlank()) {
                    entity.setCreatedAt(df.parse(record.get(EntitySheetHeaders.CREATED_DATE)));
                }
                if (record.isMapped(EntitySheetHeaders.UPDATED_DATE) && !record.get(EntitySheetHeaders.UPDATED_DATE).isBlank()) {
                    entity.setUpdatedAt(df.parse(record.get(EntitySheetHeaders.UPDATED_DATE)));
                }
                if (record.isMapped(EntitySheetHeaders.REVIEWED_DATE) && !record.get(EntitySheetHeaders.REVIEWED_DATE).isBlank()) {
                    entity.setReviewedAt(df.parse(record.get(EntitySheetHeaders.REVIEWED_DATE)));
                }

                Map<String, Object> props = new HashMap<>();
                if (record.isMapped(EntitySheetHeaders.ADDITIONAL_PROPERTIES) && !record.get(EntitySheetHeaders.ADDITIONAL_PROPERTIES).isBlank()) {
                    ObjectMapper mapper = new ObjectMapper();
                    props.putAll(mapper.readValue(record.get(EntitySheetHeaders.ADDITIONAL_PROPERTIES), new TypeReference<Map<String, Object>>() {
                    }));
                }
                ...
                if ("competency".equalsIgnoreCase(type)) {
                    ...
                    String[][] levelColumns = {
                            {EntitySheetHeaders.COMPETENCY_LEVEL_1_NAME, EntitySheetHeaders.COMPETENCY_LEVEL_1_DESCRIPTION},
                            {EntitySheetHeaders.COMPETENCY_LEVEL_2_NAME, EntitySheetHeaders.COMPETENCY_LEVEL_2_DESCRIPTION},
                            {EntitySheetHeaders.COMPETENCY_LEVEL_3_NAME, EntitySheetHeaders.COMPETENCY_LEVEL_3_DESCRIPTION},
                            {EntitySheetHeaders.COMPETENCY_LEVEL_4_NAME, EntitySheetHeaders.COMPETENCY_LEVEL_4_DESCRIPTION},
                            {EntitySheetHeaders.COMPETENCY_LEVEL_5_NAME, EntitySheetHeaders.COMPETENCY_LEVEL_5_DESCRIPTION}
                    };
...
//        log.info("Parsed {} rows from file", rows.size());
    ...
    public MasterEntity update(Integer id, MasterEntity entity) {
        MasterEntity existing = masterEntityRepository.findById(id)
    ...
    public List<MasterEntity> search(String keyword) {
        List<MasterEntityDocument> documents = elasticSearchEntityRepository.findByNameContaining(keyword);
