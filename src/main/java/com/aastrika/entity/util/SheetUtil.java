...
public class SheetUtil {
    ...
    private List<CompetencyLevel> getCompetencyListByEntity(EntitySheetRow entitySheetRow, MasterEntity masterEntity) {
        List<CompetencyLevel> competencyLevelList = new ArrayList<>();

        for (int levelIndex = 1; levelIndex <= entitySheetProperties.getCompetencyLevelSize(); levelIndex++) {
            CompetencyData competencyData = getCompetencyData(entitySheetRow, levelIndex);

            if (competencyData != null && !StringUtil.isBlank(competencyData.name)
                    && !StringUtil.isBlank(competencyData.description)) {

                CompetencyLevel competencyLevel = CompetencyLevel.builder()
    ...
    private CompetencyData getCompetencyData(EntitySheetRow entitySheetRow, int levelIndex) {
        return switch (levelIndex) {
            case 1 -> new CompetencyData(entitySheetRow.getCompetencyLevel1Name(), entitySheetRow.getCompetencyLevel1Description());
            case 2 -> new CompetencyData(entitySheetRow.getCompetencyLevel2Name(), entitySheetRow.getCompetencyLevel2Description());
            case 3 -> new CompetencyData(entitySheetRow.getCompetencyLevel3Name(), entitySheetRow.getCompetencyLevel3Description());
            case 4 -> new CompetencyData(entitySheetRow.getCompetencyLevel4Name(), entitySheetRow.getCompetencyLevel4Description());
            case 5 -> new CompetencyData(entitySheetRow.getCompetencyLevel5Name(), entitySheetRow.getCompetencyLevel5Description());
            default -> null;
        };
    }

    private static final Map<String, BiConsumer<EntitySheetRow, String>> FIELD_SETTERS = Map.of(
            "entity_id", EntitySheetRow::setEntityId,
            "name", EntitySheetRow::setName,
            "description", EntitySheetRow::setDescription,
            "code", new BiConsumer<EntitySheetRow, String>() {
                @Override
    ...
    public List<EntitySheetRow> mapSheetToEntitySheetRow(@NonNull List<CSVRecord> csvRecords) {
        ...
        for (CSVRecord csvRecord : csvRecords) {
            ...
            for (Map.Entry<String, String> headerFieldEntry : entitySheetProperties.getHeaderFieldMappings().entrySet()) {
