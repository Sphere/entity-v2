...
public class RowProcessingResult {
  ...
  @Builder.Default private List<RowStatus> rowStatuses = new ArrayList<>();
  ...
  public List<RowStatus> getFailedRows() {
    return rowStatuses.stream().filter(r -> !r.isSuccess()).toList();
  }
  ...
  public static class RowStatus {
    ...
    public static RowStatus success(int rowNumber) {
      return RowStatus.builder().rowNumber(rowNumber).success(true).build();
    }

    public static RowStatus failure(
        int rowNumber, List<String> missingFields, String errorMessage) {
